...
import torch
import gc # Added 'gc' import for memory management
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig

st.set_page_config(page_title="Medical Chatbot - Mistral", page_icon="🏥")

st.title("🏥 Medical Chatbot - Mistral 7B (4-bit)") # Updated title for clarity

model_path = "./models/mistral-7b"

# Load model using session_state
if 'model' not in st.session_state:
    with st.spinner("🔄 Loading Mistral 7B model (4-bit, NF4)... Please wait"): # Updated spinner
        try:
            ...
            load_in_4bit=True,                   # Set to 4-bit
...
with st.sidebar:
    ...
    st.info("""
    **Model:** Mistral 7B Instruct v0.2 (**4-bit** QLoRA)  
    **Source:** Local Files  
    **GPU:** NVIDIA RTX 4050
    """) # Updated info to match 4-bit config
...
if prompt := st.chat_input("Ask your medical question..."):
    # Check if model is loaded before proceeding with generation
    if 'model' not in st.session_state:
        st.error("Model is not loaded. Please wait for the model to load or click the 'Unload Model' button to free VRAM and try again.")
        st.stop()
...
st.sidebar.caption("💡 Mistral 7B provides excellent medical responses")